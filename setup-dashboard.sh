...
# Copy dashboard files to Hetzner server
echo "Copying dashboard files to Hetzner server..."
scp -r dashboard ${HETZNER_USER}@${HETZNER_IP}:/tmp/

# Set up the web server on Hetzner
echo "Setting up web server on Hetzner..."
ssh ${HETZNER_USER}@${HETZNER_IP} "cd /tmp/dashboard && ./setup-server.sh && cp -r * /var/www/html/ && rm -rf /tmp/dashboard"
